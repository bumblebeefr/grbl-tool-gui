...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, options) {
    ...
    var docStart = (options.value && typeof options.value == "string") ? 0 : options.value.first;
  ...
  function visibleLines(display, doc, viewPort) {
    ...
    if (typeof viewPort == "number"){top = viewPort;}else if (viewPort) {top = viewPort.top; height = viewPort.bottom - viewPort.top;}
  ...
  function updateDisplay(cm, changes, viewPort, forced) {
    ...
    for (;;) {
      ...
      if (viewPort){viewPort = Math.min(cm.display.scroller.scrollHeight - cm.display.scroller.clientHeight,
    ...
    if (sawCollapsedSpans) {
		for (var i = 0; i < intact.length; ++i) {
		    ...
		    while (merged = collapsedSpanAtEnd(getLine(doc, range.to - 1))) {
		      ...
		      if (newTo > range.from){range.to = newTo;}else { intact.splice(i--, 1); break; }
		    }
		  }
	}
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (markers) {
		for (var k = 0; k < cm.options.gutters.length; ++k) {
          ...
          if (found){gutterWrap.appendChild(elt("div", [found], "CodeMirror-gutter-elt", "left: " +
  ...
  function coordsChar(cm, x, y) {
    ...
    for (;;) {
      ...
      if (merged && (found.ch > mergedPos.from.ch || found.ch == mergedPos.from.ch && found.xRel > 0)){lineNo = mergedPos.to.line;} else {
		return found;
	}
    }
  }
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (how == null){how = "add";}
    if (how == "smart") {
      if (!cm.doc.mode.indent){how = "prev";} else {
  ...
  function changeLine(cm, handle, op) {
    ...
    if (op(line, no)){regChange(cm, no, no + 1);} else {
		return null;
	}
    return line;
  }
  ...
  CodeMirror.prototype = {
    ...
    addLineClass: operation(null, function(handle, where, cls) {
      return changeLine(this, handle, function(line) {
        ...
        if (!line[prop]){line[prop] = cls;}else if (new RegExp("(?:^|\\s)" + cls + "(?:$|\\s)").test(line[prop])) {
			return false;
		} else {line[prop] += " " + cls;}
        return true;
      });
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.addToHistory){addToHistory(doc, {from: from, to: to, origin: "markText"},
  ...
  function elt(tag, content, className, style) {
    ...
    if (typeof content == "string"){setTextContent(e, content);}else if (content) {
  ...
  var bidiOrdering = (function() {
    ...
    return function(str) {
      ...
      for (var i = 0, cur = outerType; i < len; ++i) {
        var type = types[i];
        if (type == "1" && cur == "r"){types[i] = "n";}else if (isStrong.test(type)) { cur = type; if (type == "r"){types[i] = "R";} }
      }
      ...
      for (var i = 1, prev = types[0]; i < len - 1; ++i) {
        var type = types[i];
        if (type == "+" && prev == "1" && types[i+1] == "1"){types[i] = "1";}else if (type == "," && prev == types[i+1] &&
